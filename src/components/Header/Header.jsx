...
// import { ModalViewRole } from '../SectionEmployees/ModalViewRole/ModalViewRole';
...
const AppRoutes = () => {
  let route = useRoutes([
    ...
    // { path: "/ModalViewRole", element: <ModalViewRole /> }
...
export const Header = () => {
  ...
  const handleOpaqueClick = () => {
    const SaveStateMenu = NewContext.setisOpaque(!NewContext.isOpaque);

    localStorage.setItem('NewContext.isOpaque', NewContext.setisOpaque(SaveStateMenu) )
  };
      ...
      <header className={`place-content-center items-center absolute top-[17px] left-[71px] right-0 z-50 border-gray-200 px-4 py-2.5 bg-[#ffffff] rounded-xl shadow-md w-[90%] ${NewContext.isOpaque ? 'opacity-100' : 'opacity-0'}`}>
        <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
          ...
          <div className="flex items-center lg:order-2">
            <button className="bg-[#0165FF] text-gray-800 dark:text-white hover:bg-gray-50 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800"
              onClick={() => {
                handleLoginClick();
                handleOpaqueClick();
              }}>
            ...
            <button onClick={toggleMenu} type="button" className="inline-flex items-center p-2 ml-1 text-sm text-gray-500 rounded-lg lg:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600">
          ...
          <div className={`lg:flex items-center w-full lg:w-auto lg:order-1 ${isMenuOpen ? 'block' : 'hidden'}`} id="mobile-menu-2">
            <Navbar>
